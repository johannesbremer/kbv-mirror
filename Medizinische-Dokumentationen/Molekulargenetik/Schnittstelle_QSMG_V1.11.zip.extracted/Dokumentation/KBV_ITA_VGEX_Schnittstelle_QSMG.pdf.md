...
**1. Einleitung**  Im Rahmen von QS-Vereinbarung Molekulargenetik [1] wird jährlich eine Statistik in der Arzt- praxis / Labor erstellt und an die Datenannahmestelle versendet.
...
2. Dateinamen  ***Der Dateiname basiert auf dem Dateinamenskonzept der EHD***
...
***Zeitstempel ( Datum und***  ***JJJJMMTThhmmss***
...
**4. Beschreibung der ehd-Schnittstelle**  Die Schnittstelle „QS Molekulargenetik“ basiert auf [KBV_ITA_VGEX_eHD]. Damit entspricht  die XML-Struktur vom header genau den Vorgaben der EHD-Richtlinie. Im Body-Bereich wer- den eigene Elemente definiert, die im Kapitel 4.2 beschrieben werden.
...
-Element im V-Attribut ist der Wert: „QS Molekulargenetik“ fest o *interface.nm* vorgeschrieben.
...
Das body-Element hat den benutzerdefinierten Typ **qsmg_body_typ**. Das Kindelement in die- sem Typ wird im Kapitel 4.3 beschrieben. Der Namensraum für die Kindelemente unterschei- det sich vom ehd-Namensraum und lautet wie folgt: „urn:ehd/qsmg/001“.
...
**praediktive_fragestellung** **4.3.3**

**diagnostische_fragestellung**> und enthält die Daten: **behandlungs-** Behandlungsfälle mit prädiktiver Fragestellung werden hier dokumentiert. Das Element gleicht  dem Aufbau von < **fall_anzahl, pathologischer_befund_anzahl, unauffaelliger_befund_anzahl** und **nichtbeurteilba-** **rer_befund_anzahl**

**diagnostische_fragestelungl**>, bezie-Alle Angaben in den Kindelementen sind ähnlich wie in < hen sich jedoch auf die Behandlungsfälle mit prädiktiver Fragestellung.

**vorgeburtliche_fragestellung 4.3.4**

**diagnostische_fragestellung**> und enthält die Daten: **behandlungs-** Behandlungsfälle mit vorgeburtlicher Fragestellung werden hier dokumentiert. Das Element  gleicht dem Aufbau von < **fall_anzahl, pathologischer_befund_anzahl, unauffaelliger_befund_anzahl** und **nichtbeurteilba-** **rer_befund_anzahl**
...
**4.3.5 l eistung_liste/leistung**  Die erbrachten Leistungen mit Gebührenordnungsziffer (GOP) werden hier dokumentiert. Das  Element **leistung**kann mehrfachvorkommen. Das Element enthäl  te: **gop, gop_anzahl, pathologischer_befund_anzahl, unauffaelliger_befund_anzahl** **teilbarer_befund_anzahl** und optional **fachgruppe_einsender_liste**
...
**4.3.6 quotient_pathologisch**  Aus der Anzahl der Behandlungsfälle mit pathologischem Befund und den zugehörigen er- brachten Leistungen wird ein Quotient berechnet. Die Kindelemente sind  **fund_anzahlquotient.**
...
**4.3.7 quotient_unauffaellig**  Der Aufbau dieses Elements ist mit dem Element  tisch. Es werden nur die Behandlungsfälle mit unauffälligem Befund berücksichtigt, d.h. aus  der Anzahl der Behandlungsfälle mit unauffälligem Befund und den zugehörigen erbrachten  Leistungen wird der Quotient berechnet.
...
**4.3.8 quotient_nichtbeurteilbar**  Der Aufbau dieses Elements ist mit dem Element  tisch. Es werden nur die Behandlungsfälle mit nicht beurteilbarem Befund berücksichtigt: d.h.  aus der Anzahl der Behandlungsfälle mit nicht beurteilbarem Befund und den zugehörigen  erbrachten Leistungen wird der Quotient berechnet.
...
**4.3.9 unterauftrag_anzahl**  Die Anzahl (nonNegativeInteger) der Behandlungsfälle, für die ein Unterauftrag existiert, wird  hier im V-Attribut angegeben.

**4.3.10 einsender_lanr_anzahl**  Die Anzahl der Ärzte, die eine Probe zur genetischen Analyse geschickt haben (Überwei- sung), wird ermittelt. Diese Anzahl (nonNegativeInteger) aller LANR n, für die eine Überwei-
...
**einsendungen_anzahl 4.3.12.2**

Die Anzahl (nonNegativeInteger) der Einsendungen wird hier im V-Attribut angegeben.

**Produkt1_teilnahmehaufigkeit** **4.3.13**  Das Produkt (nonNegativeInteger) aus ringversuchspflichtiger Untersuchung und der Anzahl  der Teilnahmehäufigkeit im Jahr für die Untersuchung wird hier im V-Attribut angegeben.

**Produkt2_ringversuchszertifikat** **4.3.14**  Das Produkt (nonNegativeInteger) aus ringversuchspflichtiger Untersuchung und erteilter Zer- tifikate im Jahr für diese Untersuchung wird hier im V-Attribut angegeben.
