# zur Prüfung der Dokumentationsdaten

# des strukturierten Behandlungsprogramms

**Grundlage:**

**Stand der Bearbeitung:**

**Version anzuwenden ab:**

- Anlage

# Plausibilitätsrichtlinie 18* -

# Depression

**Anlage 18 der DMP-A-RL**

**gültig ab 01.01.2020**

# Version: 1.0

**30.04.2020**

**01.04.2021**


---

**Anlage 18* Depression**

| Laufende | Dokumentationsparameter | Ausprägung | Plausibilitäten |
| Ist in den indikationsübergreifenden Dokumentationsparametern gemäß | | | |
|---|---|---|---|
| Anamnese- und Befunddaten |  | | |
| 1 | Summenwert | – | Pflichtfeld Nummerische Angabe Die Angabe erfolgt ohne Nachkommastelle. Die Angabe muss gleich oder größer „0“ |
| 2 | aktuellen depressiven Episode | Leicht / Mittelgradig / Schwer | Bedingtes Genau eine Angabe ist zulässig Eine Angabe ist erforderlich, wenn die Eine Angabe |
| 3 | aktuellen depressiven Episode | Aktuell keine depressive Episode / Leicht / Bedingtes Pflichtfeld | Genau eine Angabe ist zulässig |

sind die nachstehenden Felder der Anlage 18* entsprechend den Festlegungen der Plausibilitätsregeln auszufüllen Aktuelle Symptomatik PHQ-9 Zahl (0 Schweregrad Beginn Anlage 2* in Feld 10 mindestens die Angabe „  27, ganzzahlig) Schweregrad Beginn Depression“  erfolgt, und kleiner  oder gleich 27  sein. Anlage 18* im Rahmen Erstdokumentation auszufüllen ist. im Rahmen Folgedokumentation ist nicht zulässig einer einer #  Datensatz für die indikationsspezifische Dokumentation


---

| Laufende | Dokumentationsparameter | Ausprägung | Plausibilitäten |
|---|---|---|---|
|  |  |  | Eine Angabe ist erforderlich, Eine Angabe |
| 4 | Episode | Mehr als zwei Jahre persistierend / Weniger | Bedingtes Pflichtfeld Genau eine Angabe ist zulässig Eine Angabe ist immer erforderlich, Eine Angabe ist erforderlich, wenn die Eine Angabe |
| 5 | Suizidalität eingeschätzt | Ja / Nein | Pflichtfeld Genau eine Angabe ist zulässig |

Nummer Anlage 18* Rahmen einer Folgedokumentation auszufüllen ist. Rahmen einer Erstdokumentation ist nicht zulässig Dauer aktuellen depressiven die Anlage 18* Rahmen einer Erstdokumentation auszufüllen ist Anlage 18* Rahmen einer Folgedokumentation auszufüllen ist und in Feld 3 Leicht “ oder Mittelgradig “ oder Schwer “ erfolgt ist ist nicht zulässig, wenn die Anlage 18* Rahmen einer Folgedokumentation auszufüllen ist und in Feld 3 „Aktuell ke depressive Episode“  erfolgt ist.

---

| Laufende | Dokumentationsparameter | Ausprägung | Plausibilitäten |
|---|---|---|---|
| Nicht-medikamentöse und medikamentöse Behandlung |  | | |
| 6 | Aktuelle Psychotherapie | Ja / Nein / Geplant / Nicht gewünscht | Pflichtfeld |
| 7 | Aktuelle | Aktuell andauernd, seit weniger als | Pflichtfeld |
| Schulungen |  | | |
| 8 | Bereits | Ja / Nein | Bedingtes Pflichtfeld . Eine Angabe ist erforderlich, Eine Angabe |
| 9 | Depressionsspezifische | Ja / Nein | Pflichtfeld Genau eine Angabe ist zulässig |

Nummer medikamentöse Therapie mit Antidepressiva vor Einschreibung in das DMP depressionsspezifischen teilgenommen einer empfohlen (bei aktueller Dokumentation) Monaten / Aktuell andauernd, Fortführung über neun Monate hinaus / Nein / Vor Ablauf von neun Monaten abgebrochen Nach mehr abgeschlossen neun / Kontraindikation / Nicht Monaten Genau eine Angabe ist zulässig. Genau eine Angabe ist zulässig. Anlage 18* im Rahmen Erstdokumentation auszufüllen ist. im Rahmen Folgedokumentation ist nicht zulässig wenn die einer einer

---

| Laufende | Dokumentationsparameter | Ausprägung | Plausibilitäten |
|---|---|---|---|
|  |  |  |  |
| 10 | Depressionsspezifische | Digitales | Eine Angabe ist erforderlich, wenn die Eine Angabe im Rahmen einer |

Nummer Schulung wahrgenommen Selbstmanagement Präsenzgruppenschulung aktuell nicht möglich Dokumentation keine Schulung empfohlen -Programm/ Bedingtes Pflichtfeld Nein / War Nur eine Angabe ist möglich Bei letzter Anlage 1 8* im Rahmen einer Folgedokumentation auszufüllen ist. Erstdokumentation ist nicht zulässig