	...
	then
		...
		then
			...
			then
				...
				KONFIG_FD=${XPM_INSTALLATION_DIR}/Konfig/konfigFD.xml
			else
				FEHLER
			fi
		else
...
${XPM_JAVA_CALL} de.kbv.xpm.modul.dmp.bkr.start.StartKonsoleAsatz -s -e -m -c ${KONFIG_ED}

echo Pruefe Folgedokumentation

${XPM_JAVA_CALL} de.kbv.xpm.modul.dmp.bkr.start.StartKonsoleAsatzVerlaufsDoku -s -e -m -c ${KONFIG_FD}
else
	echo " "
fi
