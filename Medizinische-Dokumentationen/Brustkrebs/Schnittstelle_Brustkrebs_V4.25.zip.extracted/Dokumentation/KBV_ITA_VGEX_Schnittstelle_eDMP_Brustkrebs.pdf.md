...
19  20  20  20  21  21  21  22  22  22 23  23  24  25  26  26  27
...
6.1 Sektion (section) (FD)  6.1.1  6.1.2  6.1.3  6.1.4   6.1.5 6.1.6
...
42  43  43  44  44  45  45  46  47  47  48  48 48  49  49  50  50
...
lich für die elektronische Dokumentation benutzt. Diese Schnittstellenbeschreibung wird ausschließ
...
-Dokumentation zweimal abzugeben: Erstmalig bei der Es besteht die Möglichkeit für den Arzt, die Erst Einschreibung der Patientin in das DMP-Programm. Für den Fall, dass danach die Operation erfolgte,  -Dokumentation Brustkrebs nachgereicht werden. können die Angaben zur Operation in einer zweiten Erst
...
Die Regeln zur Plausibilisierung sind in den jeweiligen Plausibilitätskatalogen zum DMP  Brustkrebs zu  bilitätskataloge sind die Spitzenverbände entnehmen. Herausgeber und Verantwortlicher für diese Plausi der Krankenkassen [52].

HL7-Schemata und die KBV-Prüfschemata Die XML-Dateien werden gegen die durch die KBV erweiterten  *DMP_Brustkrebs_ED.xsd* und *DMP_Brustkrebs_FD.xsd* geprüft. Erst danach kann gesagt werden, ob eine  Datei und somit die Erst- bzw. Folge-Dokumentation fehlerfrei ist.
...
Für die Erstdokumentation: E  **F**olgedokumentation: F  Für die Gefolgt von zwei Buchstaben für die Kennzeichnung des DMP für Brustkrebs: BK
...
Alle Schemata, die in dieser Schnittstellenbeschreibung beschrieben werden, sind im Ordner „Schema“ in  jedem Prüfmodul enthalten. Das Schema des Elements  DMP_Brustkrebs_ED.xsd. Das Schema des Elements <levelone> der Folgedokumentation Brustkrebs heißt  DMP_Brustkrebs_FD.xsd.
...
Das Element <clinical_document_header> wird allgemein für alle DMP-Dokumentationen in dem  Header“ [Dokument „Schnittstellenbeschreibung DMP KBV_ITA_VGEX_Schnittstelle_eHeader] beschrieben.
...
krebses“, Primärtumors/kontralateralen Brustkrebses“, „Aktueller Befundstatus des Primärtumors/kontralateralen  Brustkrebses“, „Behandlung des Primärtumors/kontralateralen Brust Befunde und Therapie von  „Sonstige Befunde“ und „Behandlungsplanung“, die jeweils in einem <paragraph>-Fernmetastasen Element untergebracht sind.

**5.1.1** **caption**

-Block> weggelassen. Die content-Elemente der einzelnen Abschnitte

Zu jedem <paragraph>-Element muss ein Kindelement <caption> mit der Abschnittsüberschrift und ein  Kindelement <content> mit den konkreten Daten übertragen werden. Enthält ein Abschnitt keine Daten, so  wird der entsprechende <paragraph werden in den nächsten Kapiteln erläutert.
...
-ssu angegeben. Das Element  Das Element <content> enthält das Kindelement <local_markup>, mit welchem eine sciphox-ssu verwendet  werden kann. Die eigentlichen Daten werden mit Hilfe der sciphox *ignore* und *descriptor*. Das Attribut  hat den festen <local_markup> hat die erforderlichen Attribute *ignore* Wert “all”. Um zu kennzeichnen, dass SCIPHOX-Elemente verwendet werden, ist für das *descriptor*-Attribut  der feste Wert „sciphox“ vorgeschrieben.

Für die Darstellung von Daten des Brustkrebses (Erst- und Folgedokumentation) in XML wird ausschließlich  verwendet. Das Element <sciphox-ssu> hat drei Attribute, die mit den festen die Sciphox-SSU *observation* Werten vorbelegt sind: *type* =“observation“, *country*=“de“, *version*=“v1“. Damit wird gekennzeichnet, dass  die Sciphox-SSU *observation* in Version v1 verwendet wird. Der Coderahmen für das Element <content> mit  sieht demnach folgendermaßen ausSciphox-SSU *observation*
...
*DN*-Attribut. Als Wert werden die Parameter aus dem Datensatz Das Element <Parameter> enthält nur das  (z.B. „Primärtumor Datum der histologischen Sicherung“), zu welchen eine Angabe gemacht werden muss,  angegeben. Die einzelnen Angaben werden im jeweiligen Element <Ergebnistext>, <Ergebniswert>,  <Normbereich> und <Zeitpunkt_dttm> untergebracht.
...
-Code zum Element <Lokalisation> sieht  Das Element <Lokalisation> enthält nur das V-Attribut. In diesem Element wird die Position eines  Körperteils (z.B. „rechts“ bzw. „links“) angegeben. Der XML folgendermaßen aus:
...
<Obergrenze> verwendet. Weitere  In diesem Element wird der Normbereich (z.B. obere und/oder untere Wertgrenze) zu einem Wert  angegeben. Um die Obergrenze anzugeben, wird das Kindelement  Kindelemente wie <Untergrenze> werden nicht benutzt, daher enthält das Element <Normbereich> nur das  Pflicht-Kindelement <Obergrenze>.

*V*-, das *DN*- und das *U*-Attribut, die zwingend erforderlich sind. Das Das Element <Obergrenze> enthält das  *DN*- und das *U* Attribut sind mit festen Werten vorbelegt: DN=“oberer Normwert des Labors“, U=“ “. Der  XML-Code zum Element <Normbereich> sieht folgendermaßen aus:
...
Das Das Element <Zeitpunkt_dttm> enthält das V-Attribut, in das ein Datum eingetragen werden kann.  Datumsformat entspricht dem Pattern: JJJJ-MM-TT, JJ-MM oder nur JJJJ. Wobei J=Jahr, M=Monat und  T=Tag entsprechen. Der XML-Code zum Element <Zeitpunkt_dttm> sieht folgendermaßen aus:
...
Sicherung“ und ein Element  Das Element <sciphox:Beobachtung> hat als Kindelemente das Element <sciphox:Parameter> mit dem  konstanten Wert „Kontralateraler Brustkrebs Datum der histologischen  <sciphox:Zeitpunkt_dttm>. Wird keine Angabe zu diesem Parameter gemacht, entfällt das Element  <sciphox:Beobachtung>.
...
Sicherung“ und ein Element  Das Element <sciphox:Beobachtung> hat als Kindelemente das Element <sciphox:Parameter> mit dem  konstanten Wert „Lokoregionäres Rezidiv Datum der histologischen  das Element <sciphox:Zeitpunkt_dttm>. Wird keine Angabe zu diesem Parameter gemacht, entfällt  <sciphox:Beobachtung>.
...
Fernmetastasen  Das Element <sciphox:Beobachtung> hat als Kindelemente das Element <sciphox:Parameter> mit dem  konstanten Wert „Datum der diagnostischen Sicherung von Fernmetastasen“ und ein  Element <sciphox:Zeitpunkt_dttm>. Wird keine Angabe zu diesem Parameter gemacht, entfällt das Element  <sciphox:Beobachtung>.
...
Anamnese und Behandlungsstatus des Dieses Kapitel beschreibt den Abschnitt  Primärtumors/kontralateralen Brustkrebses

*observation* verwendet. Der Aufbau dieser SSU ist in Kapitel Im Element <content> wird die Sciphox-SSU  5.1.3 dargestellt. Diese SSU enthält genau ein Kindelement <sciphox:Beobachtungen>. Das Element  <sciphox:Beobachtungen> enthält maximal ein Kindelement <sciphox:Beobachtung> mit dem fest  vorgegebenen <sciphox:Parameter>-Element: Operative Therapie. Wird keine Angabe zu diesem Parameter  gemacht, entfällt der Abschnitt.
...
<sciphox:Ergebnistext> im *V*-Attribut die Feldbezeichnung gemäß Bei diesem Parameter enthält das Element  Tabelle 5. Wenn bei diesem Parameter mehrere Werte ausgewählt wurden, können mehrere Elemente  <sciphox:Ergebnistext> mit entsprechenden Werten angegeben werden. Wird keine Angabe zu diesem  Parameter gemacht, entfällt das Element <sciphox:Beobachtung>.
...
Werte bei Ergebnistext (Operative Therapie ) (ED) **Tabelle 5:**

**Abschnitt „** **5.1.6** **Aktueller Befundstatus des Primärtumors/kontralateralen Brustkrebses**

Dieses Kapitel beschreibt den Abschnitt „Aktueller Befundstatus des Primärtumors/kontralateralen  Brustkrebses

Im Element <content> wird die Sciphox-SSU *observation* verwendet. Der Aufbau dieser SSU ist in Kapitel  5.1.3 dargestellt. Diese SSU enthält genau ein Kindelement <sciphox:Beobachtungen>. Das Element  <sciphox:Beobachtungen> enthält maximal fünf Kindelemente <sciphox:Beobachtung> mit den jeweils fest  TNM-Klassifizierung“, „T“, „N“, „M“, vorgegebenen <sciphox:Parameter>-Elementen:  re [IRS])“„Hormonrezeptorstatus Östrogen und/oder Progesteron (gemäß Immunreaktiver Sco
...
*V*-Attribut die Feldbezeichnung Bei diesem Parameter enthält das Element <sciphox:Ergebnistext> im  gemäß Tabelle 7.
...
*V*-Attribut die Feldbezeichnung Bei diesem Parameter enthält das Element <sciphox:Ergebnistext> im  gemäß Tabelle 8.
...
*V*-Attribut die Feldbezeichnung Bei diesem Parameter enthält das Element <sciphox:Ergebnistext> im  gemäß Tabelle 9.
...
*V*-Attribut die Feldbezeichnung Bei diesem Parameter enthält das Element <sciphox:Ergebnistext> im  gemäß Tabelle 10.
...
**Behandlung des Primärtumors/kontralateralen Brustkrebses** **5.1.7** **Abschnitt „**
...
Im Element <content> wird die Sciphox-SSU *observation* verwendet. Der Aufbau dieser SSU ist in Kapitel  5.1.3 dargestellt. Diese SSU enthält genau ein Kindelement <sciphox:Beobachtungen>. Das Element  <sciphox:Beobachtungen> enthält maximal vier Kindelemente <sciphox:Beobachtung> mit den jeweils fest  „Aktuelle adjuvante endokrine Therapie“, vorgegebenen <sciphox:Parameter>-Elementen:  „Nebenwirkungen der aktuellen adjuvanten endokrinen Therapie“, „Fortführung der adjuvanten  endokrinen Therapie“ und “Bei Patientinnen unter adjuvanter Therapie mit Aromataseinhibitoren, sofern  töse Therapie einer Osteoporose besteht: DXA Befund“.die Absicht für eine spezifische medikamen
...
Bei diesem Parameter enthält das Element <sciphox:Ergebnistext> im *V*-Attribut die Feldbezeichnung  gemäß Tabelle 11. Wenn bei diesem Parameter mehrere Werte ausgewählt wurden, können mehrere  erden. Wird keine Angabe zu Elemente <sciphox:Ergebnistext> mit entsprechenden Werten angegeben w diesem Parameter gemacht, entfällt das Element <sciphox:Beobachtung>.
...
Wert bei Ergebnistext (V="...") abgebrochen Aktuell andauernd, seit weniger als 5 Jahren Aktuell andauernd, Fortführung über Jahre hinaus <sciphox:Beobachtung> <sciphox:Parameter DN="Bei Patientinnen unter adjuvanter Therapie mit Aromataseinhibitoren, sofern die Absicht für eine spezifische medikamentöse Therapie einer Osteoporose besteht: DXA-Befund"/> <sciphox:Ergebnistext V="Auffällig"/> Wert bei Ergebnistext (V="...")Im Element <content> wird die Sciphox-SSU *observation* verwendet. Der Aufbau dieser SSU ist in Kapitel  5.1.3 dargestellt. Diese SSU enthält genau ein Kindelement <sciphox:Beobachtungen>. Das Element
...
) (ED) **XML-Code 30:** content (Befunde und Therapie von Fernmetastasen

Lokalisation von Fernmetastasen

<sciphox:Ergebnistext> im *V*-Attribut die Feldbezeichnung Bei diesem Parameter enthält das Element gemäß Tabelle 15. Wenn bei diesem Parameter mehrere Werte ausgewählt wurden, können mehrere  Wird keine Angabe zu Elemente <sciphox:Ergebnistext> mit entsprechenden Werten angegeben werden. diesem Parameter gemacht, entfällt das Element <sciphox:Beobachtung>.
...
rden. Wird keine Angabe zu  <sciphox:Beobachtung>.
...
*V*-Attribut die Feldbezeichnung Bei diesem Parameter enthält das Element <sciphox:Ergebnistext> im  gemäß Tabelle 20. Wenn bei diesem Parameter mehrere Werte ausgewählt wurden, können mehrere  Elemente <sciphox:Ergebnistext> mit entsprechenden Werten angegeben werden.
...
) (ED) **Tabelle 20:** Werte bei Ergebnistext ( Z. n. besonders kardiotoxischer Tumortherapie

Körpergröße

<sciphox:Ergebniswert> im *V*-Attribut die „Körpergröße“ und im Bei diesem Parameter enthält das Element  *U*-Attribut den fest vorgeschriebenen Wert „cm“
...
<sciphox:Ergebniswert> im *V*-Attribut das „Körpergewicht“ und Bei diesem Parameter enthält das Element  im *U* Attribut den fest vorgeschriebenen Wert „kg“.
...
*V*-Attribut die Feldbezeichnung Bei diesem Parameter enthält das Element <sciphox:Ergebnistext> im  gemäß Tabelle 21.
...
Werte bei Ergebnistext (Dokumentationsintervall) (ED) **Tabelle 21:**
...
der „Folge Dokumentation“ erläutert. Die Beschriftungen der In diesem Kapitel wird der Aufbau des bodys  sseren Übersicht in den Abbildungen, der XML-Code-Angaben und der Tabellen enthalten zur be Verzeichnissen das in Klammern gesetzte Kürzel FD für Folge-Dokumentation. Im Element <body> der XML- Datei werden die eigentlichen Untersuchungsdaten aufgeführt. Das Element <body> selbst enthält ein  Element <section>.
...
„Einschreibung erfolgte wegen“, „Behandlungsstatus nach operativer Eine Sektion enthält die Abschnitte:  Therapie des Primärtumors/kontralateralen Brustkrebses (adjuvante Therapie)“, „Seit der letzten  „Sonstige Befunde „Behandlung bei fortgeschrittener Dokumentation neu aufgetretene Ereignisse“, und „Behandlungsplanung“Erkrankung (lokoregionäres Rezidiv/Fernmetastasen, die jeweils in einem  <paragraph>-Element untergebracht sind.

**6.1.1** **caption**

r Abschnittsüberschrift und ein Zu jedem <paragraph>-Element muss ein Kindelement <caption> mit de Kindelement <content> mit den konkreten Daten übertragen werden. Enthält ein Abschnitt keine Daten, so  -Block weggelassen. Die <content>-Elemente der einzelnen Abschnitte wird der entsprechende <paragraph> werden in den nächsten Kapiteln erläutert.

sind, sieht der Coderahmen für das Element <section> wie Wenn sämtliche Abschnitte Daten enthalten folgt aus.
...
<paragraph> <caption_cd DN="Einschreibung erfolgte wegen"/> <content> ... </content> <paragraph> <caption_cd DN="Behandlungsstatus nach operativer Therapie des Primärtumors/kontralateralen Brustkrebses (adjuvante Therapie)"/> <content> ... </content> <paragraph> <caption_cd DN="Seit der letzten Dokumentation neu aufgetretene Ereignisse"/> <content> ... </content> <paragraph> <caption_cd DN="Sonstige Befunde"/> <content> ... </content> <paragraph> <caption_cd DN="Behandlung bei fortgeschrittener Erkrankung (lokoregionäres Rezidiv/Fernmetastasen) <content> ... </content> <paragraph> <caption_cd DN="Behandlungsplanung"/> <content> ... </content>Die Schemastruktur des Elements content bei der Folgedokumentation entspricht der im Abschnitt 5.1.2  beschriebenen Struktur.
...
Die Schemastruktur des Elements *sciphox:sciphox-ssu (observation)* Kapitel 5.1.3 zu entnehmen. Die Beschreibung der Kindelemente zur SSU  5.1.3.1 bis 5.1.3.7.

**6.1.4** **Abschnitt „Einschreibung erfolgte wegen**

Der Abschnitt „Einschreibung erfolgte wegen“ enthält im <content> <sciphox:Beobachtungen>. Das Element <sciphox:Beobachtungen> enthält ein Kindelement  <sciphox:Beobachtung> mit dem fest vorgegebenen <sciphox:Parameter> wegen“. Die verwendete Sciphox-SSU <observation> im <content>-Element ist im Kapitel 5.1.3  beschrieben.
...
<sciphox:Parameter DN="Aktuelle adjuvante endokrine Therapie "/> <sciphox:Ergebnistext V="Aromataseinhibitoren"/> Wert bei Ergebnistext (V="...") <sciphox:Parameter DN="Nebenwirkungen der aktuellen adjuvanten endokrinen Therapie <sciphox:Ergebnistext V="Nein"/> </sciphox:Beobachtung>**Tabelle 24:** Werte bei Ergebnistext ( Nebenwirkungen der aktuellen adjuvanten endokrinen  Therapie) (FD)
...
*V*-Attribut die Feldbezeichnung Bei diesem Parameter enthält das Element <sciphox:Ergebnistext> im  gemäß Tabelle 26. Wird keine Angabe zu diesem Parameter gemacht, entfällt das Element  <sciphox:Beobachtung>.
...
**XML-Code 49**: Bei Patientinnen unter adjuvanter Therapie mit Aromataseinhibitoren, sofern die  -Befund (FD) Absicht für eine spezifische medikamentöse Therapie einer Osteoporose besteht: DXA
...
Im Element <content> wird die Sciphox-SSU *observation* verwendet. Der Aufbau dieser SSU ist in Kapitel  5.1.3 dargestellt. Diese SSU enthält genau ein Kindelement < <sciphox:Beobachtungen> enthält maximal fünf Kindelemente <sciphox:Beobachtung> mit den jeweils fest  vorgegebenen <sciphox:Parameter>-Elementen: „Lokoregionäres Rezidiv  Sicherung)“, „Kontralateraler Brustkrebs (Datum des histologischen Sicherung) Fernmetastasen (Datum der diagnostischen Sicherung)“, „Symptomatisches Lymphödem“ und „Bioptische  Sicherung der viszeralen Metastasen“.
...
Lokoregionäres Rezidiv (Datum der histologischen Sicherung ) (FD) **XML-Code 51:**
...
Das Element <sciphox:Beobachtung> hat als Kindelemente das Element <sciphox:Parameter> mit dem  Kontralateraler Brustkrebs (Datum des histologischen Sicherung)“, ein Element konstanten Wert „ *V*-Attribut <sciphox:Zeitpunkt_dttm> oder ein Element <sciphox:Ergebnistext> mit der Feldbezeichnung im  gemäß Tabelle 28. Wird keine Angabe zu diesem Parameter gemacht, entfällt das Element  <sciphox:Beobachtung>.
...
Kontralateraler Brustkrebs (Datum des histologischen Sicherung) (FD) **XML-Code 52:**
...
Das Element <sciphox:Beobachtung> hat als Kindelemente das Element <sciphox:Parameter> mit dem  Lokalisation von Fernmetastasen (Datum der diagnostischen Sicherung)konstanten Wert „“ ein oder  mehrere Elemente <sciphox:Ergebnistext> mit der Feldbezeichnung im *V*-Attribut gemäß Tabelle 29 und  optional ein Element <sciphox:Zeitpunkt_dttm>. Wenn bei diesem Parameter mehrere Werte ausgewählt

emacht, entfällt das Element <sciphox:Beobachtung>.  wurden, können mehrere Elemente <sciphox:Ergebnistext> mit entsprechenden Werten angegeben  werden. Wird keine Angabe zu diesem Parameter g
...
Bei diesem Parameter enthält das Element <sciphox:Ergebnistext> im *V*-Attribut die Feldbezeichnung  gemäß Tabelle 34. Wenn bei diesem Parameter mehrere Werte ausgewählt wurden, können mehrere  angegeben werden. Wird keine Angabe zu Elemente <sciphox:Ergebnistext> mit entsprechenden Werten  diesem Parameter gemacht, entfällt das Element <sciphox:Beobachtung>.
...
<sciphox:Ergebnistext> im *V*-Attribut die Feldbezeichnung Bei diesem Parameter enthält das Element  gemäß Tabelle 35. Wenn bei diesem Parameter mehrere Werte ausgewählt wurden, können mehrere  werden. Wird keine Angabe zu Elemente <sciphox:Ergebnistext> mit entsprechenden Werten angegeben  diesem Parameter gemacht, entfällt das Element <sciphox:Beobachtung>.
...
*observation* verwendet. Der Aufbau dieser SSU ist in Kapitel Im Element <content> wird die Sciphox-SSU  5.1.3 dargestellt. Diese SSU enthält genau ein Kindelement <sciphox:Beobachtungen>. Das Element  <sciphox:Beobachtungen> enthält ein Kindelement <sciphox:Beobachtung> mit dem fest vorgegebenen  <sciphox:Parameter>-Element: „Dokumentationsintervall“.
...
*V*-Attribut die Feldbezeichnung Bei diesem Parameter enthält das Element <sciphox:Ergebnistext> im  gemäß Tabelle 36.
